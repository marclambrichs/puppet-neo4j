...
# === Parameters
#
# @param browser_allow_outgoing_connections [Boolean] Configure the policy for outgoing Neo4j Browser connections.
#
# @param browser_credential_timeout [Pattern[/[1-9][0-9]*(ms|s|m)?/]] Configure the Neo4j Browser to time out logged in users after this idle period.
#
# @param browser_remote_content_hostname_whitelist [String] Whitelist of hosts for the Neo4j Browser to be allowed to fetch content from.
#
# @param browser_retain_connection_credentials [Boolean] Configure the Neo4j Browser to store or not store user credentials.
#
# @param causal_clustering_array_block_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of ARRAY_BLOCK IDs.
# @since 3.1
#
# @param causal_clustering_catchup_batch_size [Integer] The maximum batch size when catching up (in unit of entries).
# @since 3.1
#
# @param causal_clustering_cluster_allow_reads_on_followers [Boolean] Configure if the dbms.cluster.routing.getServers() procedure should include followers as read endpoints or return only read replicas.
# @since 3.1
#
# @param causal_clustering_cluster_routing_ttl [Pattern[/[1-9][0-9]{3,}(ms|s|m)?/]] How long drivers should cache the data from the  dbms.cluster.routing.getServers() procedure.
# @since 3.1
#
# @param causal_clustering_cluster_topology_refresh [Pattern[/[1-9][0-9]{3,}(ms|s|m)?/]] Time between scanning the cluster to refresh current server’s view of topology.
# @since 3.1
#
# @param causal_clustering_disable_middleware_logging [Boolean] Prevents the network middleware from dumping its own logs.
# @since 3.1
#
# @param causal_clustering_discovery_advertised_address [Pattern[/[a-z]+(:[1-6][0-5]?[0-5]?[0-3]?[0-5]?)/]] an advertised socket address] Advertised cluster member discovery management communication.
# @since 3.1
#
# @param causal_clustering_discovery_listen_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] Host and port to bind the cluster member discovery management communication.
# @since 3.1
#
# @param causal_clustering_expected_core_cluster_size [Integer] Expected number of Core machines in the cluster.
# @since 3.1
#
# @param causal_clustering_global_session_tracker_state_size [Integer] The maximum file size before the global session tracker state file is rotated (in unit of entries).
# @since 3.1
#
# @param causal_clustering_id_alloc_state_size [Integer] The maximum file size before the ID allocation file is rotated (in unit of entries).
# @since 3.1
#
# @param causal_clustering_initial_discovery_members [Array[Pattern[/[^:]+:[1-9][0-9]{4}/]]] A comma-separated list of other members of the cluster to join.
# @since 3.1
#
# @param causal_clustering_join_catch_up_timeout [Pattern[/[1-9][0-9]{3,}(ms|s|m)?/] Time out for a new member to catch up.
# @since 3.1
#
# @param causal_clustering_label_token_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of LABEL_TOKEN IDs.
# @since 3.1
#
# @param causal_clustering_label_token_name_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of LABEL_TOKEN_NAME IDs.
# @since 3.1
#
# @param causal_clustering_last_applied_state_size [Integer] The maximum file size before the storage file is rotated (in unit of entries).
# @since 3.1
#
# @param causal_clustering_leader_election_timeout [Pattern[/[1-9][0-9]*(ms|s|m)?/]] The time limit within which a new leader election will occur if no messages are received.
# @since 3.1
#
# @param causal_clustering_log_shipping_max_lag [Integer] The maximum lag allowed before log shipping pauses (in unit of entries).
# @since 3.1
#
# @param causal_clustering_neostore_block_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of NEOSTORE_BLOCK IDs.
# @since 3.1
#
# @param causal_clustering_node_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of NODE IDs.
# @since 3.1
#
# @param causal_clustering_node_labels_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of NODE_LABELS IDs.
# @since 3.1
#
# @param causal_clustering_outgoing_queue_size [Integer] The number of messages waiting to be sent to other servers in the cluster.
# @since 3.1
#
# @param causal_clustering_property_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of PROPERTY IDs.
# @since 3.1
#
# @param causal_clustering_property_key_token_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of PROPERTY_KEY_TOKEN IDs.
# @since 3.1
#
# @param causal_clustering_property_key_token_name_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of PROPERTY_KEY_TOKEN_NAME IDs.
# @since 3.1
#
# @param causal_clustering_pull_interval Pattern[/[1-9][0-9]*(ms|s|m)/] Interval of pulling updates from cores.
# @since 3.1
#
# @param causal_clustering_raft_advertised_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] Advertised hostname/IP address and port for the RAFT server.
# @since 3.1
#
# @param causal_clustering_raft_listen_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] Network interface and port for the RAFT server to listen on.
# @since 3.1
#
# @param causal_clustering_raft_log_implementation [String] RAFT log implementation.
# @since 3.1
#
# @param causal_clustering_raft_log_prune_strategy [String] RAFT log pruning strategy.
# @since 3.1
#
# @param causal_clustering_raft_log_pruning_frequency Pattern[/[1-9][0-9]*(ms|s|m)/] RAFT log pruning frequency.
# @since 3.1
#
# @param causal_clustering_raft_log_reader_pool_size [Integer] RAFT log reader pool size.
# @since 3.1
#
# @param causal_clustering_raft_log_rotation_size Pattern[/[1-9][0-9]*[kKmMgG]?/] RAFT log rotation size.
# @since 3.1
#
# @param causal_clustering_raft_membership_state_size [Integer] The maximum file size before the membership state file is rotated (in unit of entries).
# @since 3.1
#
# @param causal_clustering_raft_messages_log_enable [Boolean] Enable or disable the dump of all network messages pertaining to the RAFT protocol.
# @since 3.1
#
# @param causal_clustering_raft_term_state_size [Integer] The maximum file size before the term state file is rotated (in unit of entries).
# @since 3.1
#
# @param causal_clustering_raft_vote_state_size [Integer] The maximum file size before the vote state file is rotated (in unit of entries).
# @since 3.1
#
# @param causal_clustering_read_replica_refresh_rate Pattern[/[1-9][0-9]*(ms|s|m)/] Read replica 'call home' frequency.
# @since 3.1
#
# @param causal_clustering_read_replica_time_to_live Pattern[/[1-9][0-9]*(ms|s|m)/] Time To Live before read replica is considered unavailable.
# @since 3.1
#
# @param causal_clustering_relationship_group_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of RELATIONSHIP_GROUP IDs.
# @since 3.1
#
# @param causal_clustering_relationship_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of RELATIONSHIP IDs.
# @since 3.1
#
# @param causal_clustering_relationship_type_token_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of RELATIONSHIP_TYPE_TOKEN IDs.
# @since 3.1
#
# @param causal_clustering_relationship_type_token_name_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of RELATIONSHIP_TYPE_TOKEN_NAME IDs.
# @since 3.1
#
# @param causal_clustering_replicated_lock_token_state_size [Integer] The maximum file size before the replicated lock token state file is rotated (in unit of entries).
# @since 3.1
#
# @param causal_clustering_schema_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of SCHEMA IDs.
# @since 3.1
#
# @param causal_clustering_state_machine_apply_max_batch_size [Integer] The maximum number of operations to be batched during applications of operations in the state machines.
# @since 3.1
#
# @param causal_clustering_state_machine_flush_window_size [Integer] The number of operations to be processed before the state machines flush to disk.
# @since 3.1
#
# @param causal_clustering_string_block_id_allocation_size [Integer] The size of the ID allocation requests Core servers will make when they run out of STRING_BLOCK IDs.
# @since 3.1
#
# @param causal_clustering_transaction_advertised_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] Advertised hostname/IP address and port for the transaction shipping server.
# @since 3.1
#
# @param causal_clustering_transaction_listen_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] Network interface and port for the transaction shipping server to listen on.
# @since 3.1
#
# @param causal_clustering_unknown_address_logging_throttle [Pattern[/[1-9][0-9]*(ms|s|m)/]] Throttle limit for logging unknown cluster member address.
# @since 3.1
#
# @param cypher_default_language_version [Enum['2.3', '3.0', '3.1', 'default']] Set this to specify the default parser (language version).
#
# @param cypher_forbid_exhaustive_shortestpath [Boolean] This setting is associated with performance optimization.
#
# @param cypher_hints_error [Boolean] Set this to specify the behavior when Cypher planner or runtime hints cannot be fulfilled.
# Type: boolean
#
# @param cypher_min_replan_interval [Pattern[/[1-9][0-9]*(ms|s|m)?/]] The minimum lifetime of a query plan before a query is considered for replanning.
#
# @param cypher_planner [Enum['COST', 'RULE', 'default']] Set this to specify the default planner for the default language version.
#
# @param cypher_statistics_divergence_threshold [Float[0.0, 1.0]] The threshold when a plan is considered stale.
#
# @param data_prefix [Pattern[/\/([^/\0]+(\/)?)+]] Determines the directory where neo4j data dirs will be installed.
#
# @param dbms_active_database [String] Name of the database to load.
#
# @param dbms_allow_format_migration [Boolean] Whether to allow a store upgrade in case the current version of the database starts against an older store version.
#
# @param dbms_backup_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] Listening server for online backups.
#
# @param dbms_backup_enabled [Boolean] Enable support for running online backups.
#
# @param dbms_checkpoint_interval_time [Pattern[/[1-9][0-9]*(ms|s|m)/]] Configures the time interval between check-points.
#
# @param dbms_checkpoint_interval_tx [Pattern[/[1-9][0-9]*/]] Configures the transaction interval between check-points.
#
# @param dbms_checkpoint_iops_limit [Integer] Limit the number of IOs the background checkpoint process will consume per second.
#
# @param dbms_connector_bolt_advertised_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] The advertised_address setting specifies the address clients should use for this connector. This is useful in a causal cluster as it allows each server to correctly advertise addresses of the other servers in the cluster.
#
...
#
# @param dbms_connector_https_enabled [Boolean] The enabled setting allows the client connector to be enabled or disabled. When disabled, Neo4j does not listen for incoming connections on the relevant port.
#
# @param dbms_connector_https_listen_address [Pattern[/[^:]+:[1-9][0-9]{4}/]] The listen_address setting specifies how Neo4j listens for incoming connections.
#
# @param dbms_connectors_default_advertised_address [String] Default hostname or IP address the server uses to advertise itself to its connectors.
# @since 3.1
#
# @param dbms_connectors_default_listen_address [String] Default network interface to listen for incoming connections.
# @since 3.1
#
# @param dbms_directories_certificates [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Directory for storing certificates to be used by Neo4j for TLS connections.
#
# @param dbms_directories_data [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Path of the data directory.
#
# @param dbms_directories_import [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Sets the root directory for file URLs used with the Cypher LOAD CSV clause.
#
# @param dbms_directories_lib [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Path of the lib directory.
#
# @param dbms_directories_logs [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Path of the logs directory.
#
# @param dbms_directories_metrics [Pattern[/(\/)?([^/\0]+(\/)?)+/]] The target location of the CSV files: a path to a directory wherein a CSV file per reported field will be written.
#
# @param dbms_directories_plugins [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Location of the database plugin directory.
#
# @param dbms_directories_run [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Path of the run directory.
#
# @param dbms_memory_heap_initial_size [Pattern[/[1-9][0-9]*[kKmMgG]?/]] Initial size of the heap
#
# @param dbms_memory_heap_max_size [Pattern[/[1-9][0-9]*[kKmMgG]?/]] Upper boundary of memory heap.
#
# @param dbms_ids_reuse_types_override [list separated by "," where items are one of [NODE, RELATIONSHIP]] Specified names of id types (comma separated) that should be reused.
# @since 3.1
#
# @param dbms_index_sampling_background_enabled [Boolean] Enable or disable background index sampling.
#
# @param dbms_index_sampling_buffer_size [Pattern[/[1-9][0-9]*[kKmMgG]?/] which is minimum 1048576, maximum 2147483647] Size of buffer used by index sampling.
# Depracated: since 3.0.3. Use dbms.index_sampling.sample_size_limit instead.

#
# @param dbms_index_sampling_sample_size_limit [Integer[1048576, 2147483647]] Index sampling chunk size limit.
#
# @param dbms_index_sampling_update_percentage [Integer] Percentage of index updates of total index size required before sampling of a given index is triggered.
#
# @param dbms_jvm_additional_commit_memory_to_process [String] Make sure that `initmemory` is not only allocated but committed to the process, before starting the database.
#
# @param dbms_jvm_additional_disable_explicit_gc [String] Disable explicit garbage collection.
#
# @param dbms_jvm_additional_ephemeral_dh_keysize [String] Expand Diffie Helman (DH) key size form default 1024 to 2048 for DH-RSA cipher suites used in serverl TLS handshakes.
#
# @param dbms_jvm_additional_hashcode Reduce probability of objects getting the same identity hash code via a race, by computing them with thread-local PRNGs.
#
# @param dbms_index_searcher_cache_size [Integer[1]] The maximum number of open Lucene index searchers.
#
# @param dbms_jvm_additional_jmxremote_access_file [String] This settings is used for exposing the JMX.
#
# @param dbms_jvm_additional_jmxremote_authenticate [Boolean] This settings is used for exposing the JMX.
#
# @param dbms_jvm_additional_jmxremote_password_file [Boolean] This settings is used for exposing the JMX.
#
# @param dbms_jvm_additional_jmxremote_port [Integer] This settings is used for exposing the JMX.
#
# @param dbms_jvm_additional_jmxremote_ssl [Boolean] This settings is used for exposing the JMX.
#
# @param dbms_jvm_additional_omit_stacktrace_in_fast_throw [String] Have common exceptions keep producing stack traces, so they can be debugged regardless of how often logs are rotated.
#
# @param dbms_jvm_additional_rmi_server_hostname [String] Some systems cannot discover host name automatically, and need rmi server hostname explicitly stated.
#
# @param dbms_jvm_additional_ssl_keystore [String] For using self-signed certificate in a test environment
#
# @param dbms_jvm_additional_ssl_keystore_password [String] For using self-signed certificate in a test environment
#
# @param dbms_jvm_additional_ssl_truststore [String] For using self-signed certificate in a test environment
#
# @param dbms_jvm_additional_ssl_truststore_password [String] For using self-signed certificate in a test environment
#
# @param dbms_jvm_additional_trust_final_non_static_fields [String] Trust that non-static final fields are really final.
#
# @param dbms_jvm_additional_unlock_experimental_vm_options [String] Allows for more optimisations and improves overall performance. Not to be used in embedded mode.
#
# @param dbms_jvm_additional_unsupported_dbms_udc_source [String] ??
#
# @param dbms_jvm_additional_use_g1gc [String] G1GC generally strikes a good balance between throughput and tail latency, without too much tuning.
#
# @param dbms_logs_debug_level [Enum['DEBUG', 'INFO', 'WARN', 'ERROR', 'NONE']] Debug log level threshold.
#
# @param dbms_logs_debug_rotation_delay [Pattern[/[1-9][0-9]*(ms|s|m)?/]] Minimum time interval after last rotation of the debug log before it may be rotated again.
#
# @param dbms_logs_debug_rotation_keep_number [Integer[1]] Maximum number of history files for the debug log.
#
# @param dbms_logs_debug_rotation_size [Pattern[/[1-9][0-9]*[kKmMgG]?/]] Threshold for rotation of the debug log.
#
...
#
# @param dbms_logs_gc_rotation_size [Pattern[/[0-9][0-9]*[kKmMgG]?/]] Size of each GC log that is kept, which is minimum 0, maximum 9223372036854775807.
#
# @param dbms_logs_http_enabled [Boolean] Enable HTTP request logging.
#
# @param dbms_logs_http_rotation_keep_number [Integer] Number of HTTP logs to keep.
#
# @param dbms_logs_http_rotation_size [Pattern[/[1-9][0-9]*[kKmMgG]?/]] Size of each HTTP log that is kept which is minimum 0, maximum 9223372036854775807
#
# @param dbms_logs_query_enabled [Boolean] Log executed queries that take longer than the configured threshold, dbms.logs.query.threshold.
#
# @param dbms_logs_query_parameter_logging_enabled [Boolean] Log parameters for executed queries that took longer than the configured threshold.
#
# @param dbms_logs_query_rotation_keep_number [Integer[1]] Maximum number of history files for the query log.
#
# @param dbms_logs_query_rotation_size [Pattern[/[0-9][0-9]*[kKmMgG]?/]] The file size in bytes at which the query log will auto-rotate which is minimum 0, maximum 9223372036854775807
#
# @param dbms_logs_query_threshold [Pattern[/[0-9]{1,}(ms|s|m)?/]] If the execution of query takes more time than this threshold, the query is logged - provided query logging is enabled. Setting this param to 0 will result in all queries being logged.
#
# @param dbms_logs_security_level [Enum['DEBUG', 'INFO', 'WARN', 'ERROR', 'NONE']] Security log level threshold.
# @since 3.1
#
# @param dbms_logs_security_rotation_delay [Pattern[/[1-9][0-9]*(ms|s|m)?/]] Minimum time interval after last rotation of the security log before it may be rotated again.
# @since 3.1
#
# @param dbms_logs_security_rotation_keep_number [Integer] Maximum number of history files for the security log.
# @since 3.1
#
# @param dbms_logs_security_rotation_size [Pattern[/[1-9][0-9]*[kKmMgG]?/]] Threshold for rotation of the security log.
# @since 3.1
#
# @param dbms_memory_pagecache_size [byte size (valid multipliers are 'k', 'm', 'g', 'K', 'M', 'G')] The amount of memory to use for mapping the store files, in bytes (or kilobytes with the 'k' suffix, megabytes with 'm' and gigabytes with 'g').
#
# @param dbms_memory_pagecache_swapper [String] Specify which page swapper to use for doing paged IO.
#
# @param dbms_mode [Enum['SINGLE', 'HA', 'ARBITER', 'CORE', 'READ_REPLICA']]
#
# @param dbms_query_cache_size [Integer[0]] The number of Cypher query execution plans that are cached.
#
# @param dbms_read_only [Boolean] Only allow read operations from this Neo4j instance.
#
# @param dbms_record_format [Enum['standard', 'high_limit']] Database record format.
#
# @param dbms_relationship_grouping_threshold [Integer[1]] Relationship count threshold for considering a node to be dense.
#
# @param dbms_rest_transaction_idle_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for idle transactions in the REST endpoint.
#
# @param dbms_security_allow_csv_import_from_file_urls [Boolean] Determines if Cypher will allow using file URLs when loading data using LOAD CSV.
#
# @param dbms_security_allow_publisher_create_token [Boolean] Set to true if users with role publisher are allowed to create new tokens.
# @since 3.1
#
# @param dbms_security_auth_cache_max_capacity [Integer] The maximum capacity for authentication and authorization caches (respectively).
# @since 3.1
#
# @param dbms_security_auth_cache_ttl [Pattern[/[1-9][0-9]*(ms|s|m)?/]] The time to live (TTL) for cached authentication and authorization info when using external auth providers (LDAP or plugin).
# @since 3.1
#
# @param dbms_security_auth_enabled [Boolean] Enable auth requirement to access Neo4j.
#
# @param dbms_security_auth_provider [Enum['native', 'ldap']] The authentication and authorization provider that contains both the users and roles.
# @since 3.1
#
# @param dbms_security_ha_status_auth_enabled [Boolean] Require authorization for access to the HA status endpoints.
#
# @param dbms_security_http_authorization_classes [Tuple[String, 1, default]] Comma-seperated list of custom security rules for Neo4j to use.
#
# @param dbms_security_ldap_authentication_cache_enabled [Boolean] Determines if the result of authentication via the LDAP server should be cached or not.
# @since 3.1
#
# @param dbms_security_ldap_authentication_mechanism [Enum['simple', 'DIGEST-MD5']] LDAP authentication mechanism.
# @since 3.1
#
# @param dbms_security_ldap_authentication_user_dn_template [String] LDAP user DN template.
# @since 3.1
#
# @param dbms_security_ldap_authorization_group_membership_attributes [Tuple[String, 1, default]] A list of attribute names on a user object that contains groups to be used for mapping to roles when LDAP authorization is enabled.
# @since 3.1
#
# @param dbms_security_ldap_authorization_group_to_role_mapping [String] An authorization mapping from LDAP group names to Neo4j role names.
# @since 3.1
#
# @param dbms_security_ldap_authorization_system_password [String] An LDAP system account password to use for authorization searches when dbms.security.ldap.authorization.use_system_account is true.
# @since 3.1
#
# @param dbms_security_ldap_authorization_system_username [String] An LDAP system account username to use for authorization searches when dbms.security.ldap.authorization.use_system_account is true.
# @since 3.1
#
# @param dbms_security_ldap_authorization_use_system_account [Boolean] Perform LDAP search for authorization info using a system account instead of the user’s own account. If this is set to false (default), the search for group membership will be performed directly after authentication using the LDAP context bound with the user’s own account.
# @since 3.1
#
# @param dbms_security_ldap_authorization_user_search_base [String] The name of the base object or named context to search for user objects when LDAP authorization is enabled.
# @since 3.1
#
# @param dbms_security_ldap_authorization_user_search_filter [String] The LDAP search filter to search for a user principal when LDAP authorization is enabled.
# @since 3.1
#
# @param dbms_security_ldap_connection_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] The timeout for establishing an LDAP connection.
# @since 3.1
#
# @param dbms_security_ldap_host [String] URL of LDAP server to use for authentication and authorization.
# @since 3.1
#
# @param dbms_security_ldap_read_timeout [Pattern[/[1-9][0-9]*(ms|s|m)?/]] The timeout for an LDAP read request (i.e. search). A value of 0 means wait for a response indefinitely.
# @since 3.1
#
# @param dbms_security_ldap_referral [String] The LDAP referral behavior when creating a connection.
# @since 3.1
#
# @param dbms_security_ldap_use_starttls [Boolean] Use secure communication with the LDAP server using opportunistic TLS.
# @since 3.1
#
# @param dbms_security_log_successful_authentication [Boolean] Set to log successful authentication events to the security log.
# @since 3.1
#
# @param dbms_security_procedures_default_allowed [String] The default role that can execute all procedures and user-defined functions that are not covered by the dbms.security.procedures.roles setting.
# @since 3.1
#
# @param dbms_security_procedures_roles [String] This provides a finer level of control over which roles can execute procedures than the dbms.security.procedures.default_allowed setting.
# @since 3.1
#
# @param dbms_shell_enabled [Boolean] Enable a remote shell server which Neo4j Shell clients can log in to.
#
...
#
# @param dbms_threads_worker_count [Integer[1,44738]] Number of Neo4j worker threads, your OS might enforce a lower limit than the maximum value specified here.
#
# @param dbms_transaction_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] The maximum time interval of a transaction within which it should be completed.
#
# @param dbms_tx_log_rotation_retention_policy [Variant[Boolean, Pattern[/[1-9][0-9]*[kKmMgG]? (size|txs)/]] Make Neo4j keep the logical transaction logs for being able to backup the database.
#
# @param dbms_tx_log_rotation_size [Pattern[/[1-9][0-9]*[kKmMgG]?/]] Specifies at which file size the logical log will auto-rotate.
#
# @param dbms_udc_enabled [Boolean] Enable the UDC extension.
#
# @param dbms_unmanaged_extension_classes [Pattern[/[a-z][a-z]*=[a-z][a-z]*/]] Comma-separated list of <classname>=<mount point> for unmanaged extensions.
#
# @param dbms_windows_service_name
#
# @param default_file [Pattern[/((\/[a-z][a-z]*)?|[a-z][a-z]*)/]] File containing environment variables
#
# @param edition [Enum['community', 'enterprise']] Pick edition of neo4j
#
# @param group [String] The group the neo4j user belongs to
#
# @param ha_allow_init_cluster [Boolean] Whether to allow this instance to create a cluster if unable to join.
#
# @param ha_branched_data_copying_strategy [Enum['branch_then_copy', 'copy_then_branch']] Strategy for how to order handling of branched data on slaves and copying of the store from the master.
#
# @param ha_branched_data_policy [Enum['keep_all', 'keep_last', 'keep_none']] Policy for how to handle branched data.
#
# @param ha_broadcast_timeout [Pattern[/[1-9][0-9]*(ms|s|m)?/]] Timeout for broadcasting values in cluster.
#
# @param ha_configuration_timeout [Pattern[/[1-9][0-9]*(ms|s|m)?/]] Timeout for waiting for configuration from an existing cluster member during
# cluster join.
#
# @param ha_data_chunk_size [Pattern[/[1-9][0-9]*[kKmMgG]?/]] Max size of the data chunks that flows between master and slaves in HA.
#
# @param ha_default_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Default timeout used for clustering timeouts.
#
# @param ha_election_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for waiting for other members to finish a role election.
#
# @param ha_heartbeat_interval [Pattern[/[1-9][0-9]*(ms|s|m)/]] How often heartbeat messages should be sent.
#
# @param ha_heartbeat_timeout [Pattern[/[1-9][0-9]*(ms|s|m)?/]] How long to wait for heartbeats from other instances before marking them as
# suspects for failure.
#
# @param ha_host_coordination [Pattern[/[^:]+:[1-9][0-9]{4}/]] Host and port to bind the cluster management communication.
#
# @param ha_host_data [Pattern[/[^:]+:[1-9][0-9]{4}/]] Hostname and port to bind the HA server.
#
# @param ha_initial_hosts [Array[Pattern[/[^:]+:[1-9][0-9]{4}/]]] A comma-separated list of other members of the cluster to join.
# Mandatory: yes
#
# @param ha_internal_role_switch_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for waiting for internal conditions during state switch, like for transactions to complete, before switching to master or slave.
#
# @param ha_join_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for joining a cluster.
#
# @param ha_learn_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for learning values.
#
# @param ha_leave_timeout [Integer] Timeout for waiting for cluster leave to finish.
#
# @param ha_max_acceptors [Integer[0]] Maximum number of servers to involve when agreeing to membership changes.
#
# @param ha_max_channels_per_slave [Integer[1]] Maximum number of connections a slave can have to the master.
#
# @param ha_paxos_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Default value for all Paxos timeouts.
#
# @param ha_phase1_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for Paxos phase 1.
#
# @param ha_phase2_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for Paxos phase 2.
#
# @param ha_pull_batch_size [Integer] Size of batches of transactions applied on slaves when pulling from master.
#
# @param ha_pull_interval [Pattern[/[1-9][0-9]*(ms|s|m)/]] Interval of pulling updates from master.
#
# @param ha_role_switch_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for request threads waiting for instance to become master or slave.
#
# @param ha_server_id [Integer] Id for a cluster instance.
# Mandatory: yes
#
# @param ha_slave_lock_timeout [Pattern[/[1-9][0-9]*(ms|s|m)/]] Timeout for taking remote (write) locks on slaves.
#
# @param ha_slave_only [Boolean] Whether this instance should only participate as slave in cluster.
#
# @param ha_slave_read_timeout [Integer[0]] How long a slave will wait for response from master before giving up.
#
# @param ha_tx_push_factor [Integer[0]] The amount of slaves the master will ask to replicate a committed transaction.
#
# @param ha_tx_push_strategy [Enum['round_robin', 'fixed_descending', 'fixed_ascending']] Push strategy of a transaction to a slave during commit.
#
...
#
# @param metrics_bolt_messages_enabled [Boolean] Enable reporting metrics about Bolt Protocol message processing.
#
# @param metrics_csv_enabled [Boolean] Set to true to enable exporting metrics to CSV files.
#
# @param metrics_csv_interval [Pattern[/[1-9][0-9]*(ms|s|m)/]] The reporting interval for the CSV files.
#
# @param metrics_csv_path [Pattern[/(\/)?([^/\0]+(\/)?)+/]] Directory path for csv output files.
#
# @param metrics_cypher_replanning_enabled [Boolean] Enable reporting metrics about number of occurred replanning events.
#
# @param metrics_enabled [Boolean] The default enablement value for all the supported metrics.
#
# @param metrics_graphite_enabled [Boolean] Set to true to enable exporting metrics to Graphite.
#
# @param metrics_graphite_interval [Pattern[/[1-9][0-9]*(ms|s|m)/]] The reporting interval for Graphite.
#
# @param metrics_graphite_server [Pattern[/[^:]+:[1-9][0-9]{4}/]] The hostname or IP address of the Graphite server.
#
# @param metrics_jvm_buffers_enabled [Boolean] Enable reporting metrics about the buffer pools.
#
# @param metrics_jvm_gc_enabled [Boolean] Enable reporting metrics about the duration of garbage collections.
#
# @param metrics_jvm_memory_enabled [Boolean] Enable reporting metrics about the memory usage.
#
# @param metrics_jvm_threads_enabled [Boolean] Enable reporting metrics about the current number of threads running.
#
# @param metrics_neo4j_causal_clustering_enabled [Boolean] Enable reporting metrics about Causal Clustering mode.
# @since 3.1
#
# @param metrics_neo4j_checkpointing_enabled [Boolean] Enable reporting metrics about Neo4j check pointing.
#
# @param metrics_neo4j_cluster_enabled [Boolean] Enable reporting metrics about HA cluster info.
#
# @param metrics_neo4j_counts_enabled [Boolean] Enable reporting metrics about approximately how many entities are in the database.
#
# @param metrics_neo4j_enabled [Boolean] The default enablement value for all Neo4j specific support metrics.
#
# @param metrics_neo4j_logrotation_enabled [Boolean] Enable reporting metrics about the Neo4j log rotation.
#
# @param metrics_neo4j_network_enabled [Boolean] Enable reporting metrics about the network usage.
#
# @param metrics_neo4j_pagecache_enabled [Boolean] Enable reporting metrics about the Neo4j page cache.
#
# @param metrics_neo4j_server_enabled [Boolean] Enable reporting metrics about Server threading info.
#
# @param metrics_neo4j_tx_enabled [Boolean] Enable reporting metrics about transactions.
#
# @param metrics_prefix [String] A common prefix for the reported metrics field names.
#
# @param release_deb [String] The name of the debian release to use, only used when manage_repo is true.
#
# @param release_rpm [String] The name of the rpm release to use, only used when manage_repo is true.
#
# @param service_enable [Boolean] Manage neo4j service.
#
# @param service_ensure [Variant[Booolean, Enum['stopped', 'running']]] Whether the service should be running.
#
# @param service_name [String] The name of the service to run.
#
# @param service_provider [Enum['debian', 'redhat', 'systemd', 'init']] The puppet service provider.
#
# @param service_ulimit [Integer] Maximum number of open file descriptors.
#
# @param service_shutdown_timeout [Integer] Number of seconds to wait for shutdown before killing the process.
#
# @param tools_consistency_checker_check_graph [Boolean] Perform checks between nodes, relationships, properties, types and tokens_
#
# @param tools_consistency_checker_check_indexes [Boolean] Perform checks on indexes.
#
# @param tools_consistency_checker_check_label_scan_store [Boolean] Perform checks on the label scan store.
#
# @param tools_consistency_checker_check_property_owners [Boolean] Perform optional additional checking on property ownership.
#
# @param unsupported_dbms_security_ldap_authorization_connection_pooling [Boolean] Set to true if connection pooling should be used for authorization searches using the system account.
#
